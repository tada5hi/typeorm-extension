...
- postgres (`postgres.ts`): the `TimeZone` goes where pg reads it. pg's `ConnectionParameters` lets a connection string's `options` override `extra.options`, and stops reading `PGOPTIONS` once `extra.options` is set, so `extra.options` gets `-c TimeZone=UTC` on top of `extra.options ?? PGOPTIONS`, and every url (top level, replication nodes) carrying `options` gets it there too. `extra.types` reads OID 1114 and 1115 through pg's own 1184 / 1185 parsers after inserting `Z` markers. `extra.Client` is a subclass (pg-pool honours the option, so it stays per pool; the base is `pg-native`'s when typeorm would switch to it, detection mirrored including its swallowed errors) serializing `Date` values as UTC, copying a plain config object rather than mutating it; a stream's values sit on `config.cursor.values`, already in pg's local form (`...+HH:MM`, the offset included), so they are re-serialized from that.
- mysql / mariadb (`mysql.ts`): `timezone: 'Z'` (reading AND parameters), `dateStrings: ['DATE']` (typeorm returns the string as is for a `date` column), and a copy of mysql2 whose `createPool` registers a `connection` listener running `SET time_zone = '+00:00'`, which mysql2 emits synchronously before handing the connection out. typeorm merges `extra` over the driver options, so settings are read from both.
- oracle (`oracle.ts`): `extra.sessionCallback` runs `ALTER SESSION SET TIME_ZONE`. node-oracledb decodes DATE and TIMESTAMP as LOCAL Dates (TSTZ and LTZ as UTC), exposes `fetchTypeHandler` only process-wide or per call, applies it to result columns only, and binds a Date from its local fields; converting a TIMESTAMP to TSTZ on fetch is refused (`NJS-119`) and fetching as a string stringifies the local Date. So `driver` becomes a proxy whose pools and standalone connections (`createPool`, `getPool`, `getConnection`) hand out proxied connections that add a handler re-reading TIMESTAMP result columns as UTC, convert TIMESTAMP OUT/INOUT out-binds after `execute` / `executeMany`, and bind Dates (untyped or typed TIMESTAMP, in `execute`, `executeMany` rows and `queryStream`) as a local Date carrying the UTC fields, which travels as a plain TIMESTAMP and leaves the column side of a comparison alone. DATE stays local on both sides because typeorm's `date` columns are calendar dates. The one inexactness is the local spring-forward gap in a DST process zone (no local Date exists for those fields), measured and documented. The proxy binds and caches methods and passes through members with a `prototype` unless replaced (`oracledb.createPool` is a plain function WITH a prototype, which is why replacement is checked first).

Settings the caller made are kept when they agree (mysql `timezone` `Z` / `+00:00`, `dateStrings: ['DATE']`, a UTC postgres `TimeZone`), built upon when they compose (pg `extra.types` for everything but timestamps, pg `extra.Client`, an oracle `sessionCallback` function running after the pin), and refused with `OptionsError.timezoneConflict` otherwise, including process-wide pg parser overrides (detected because pg registers the same function for 1114 and 1184, and for 1115 and 1185) and mysql replication. Silently skipping was the first design and was rejected: an explicit request that does nothing is the half pin in another form. Everything installed is recorded in a WeakSet (`utils.ts`), which makes a second application return the options unchanged. Only `withDataSourceTimezone` and `DataSourceTimezone` are public: the building blocks would commit to shapes still likely to move.
