...
The driver module comes from `options.driver` or typeorm's own `PlatformTools.load`, so the parser and the pool always belong to the same copy typeorm uses. Existing settings win (mysql `timezone`, a `TimeZone` in postgres `extra.options`, pg `extra.types`), which is also what makes the function idempotent. Only UTC is supported: reading any other zone needs the offset in force at each instant. The other drivers are deliberately untouched: typeorm maps cockroachdb date columns to `timestamptz` (checked against a live server: correct unpinned), better-sqlite3 stamps and reads UTC (held by `test/unit/data-source/options/timezone-sqlite.spec.ts`), mongodb stores UTC; mssql stamps `getdate()` (server OS time, no session zone to set, so only a column default such as `SYSUTCDATETIME()` fixes it) and oracle would need a pool session callback, which is not built.
