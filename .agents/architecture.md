...
`withDataSourceTimezone(options, 'UTC')` (`src/data-source/options/timezone/`) exists because a zone-less date column is stamped by the database in its SESSION zone (`now()`, `CURRENT_TIMESTAMP`) while the driver reads it, and serializes a `Date` parameter into it, in the Node PROCESS zone. Every half has to be pinned, and each was found missing once: pinning only the reader breaks a database running in local time (found downstream in authup, where a session's `createdAt` feeds OIDC `auth_time` and a future value silently satisfies `max_age`), and pinning session and reader without the writer stores app-written `Date`s on postgres and oracle in the process's wall clock, which then read back shifted. `test/integration/data-source/timezone.spec.ts` checks the stored wall clock and both value kinds, and fails with any single half removed.

- postgres: `extra.options` gains `-c TimeZone=UTC`; `extra.types` reads OID 1114 through pg's own timestamptz parser (1184) after inserting a `Z` behind the time part, so `infinity` and BC dates keep their handling; `extra.Client` is a subclass (pg-pool honours the option, so it stays per pool) whose `query` serializes `Date` parameters as UTC, the way pg's process-wide `parseInputDatesAsUTC` would. The base class is `pg-native`'s whenever typeorm would switch to it (same detection: `nativeDriver`, else a successful `PlatformTools.load('pg-native')`).
- mysql / mariadb: `timezone: 'Z'` (reading AND parameters, mysql2 uses it for both), and `driver` becomes a copy of mysql2 whose `createPool` registers a `connection` listener running `SET time_zone = '+00:00'`. mysql2 emits `connection` synchronously before it hands a new connection out and runs a connection's queries in order, which makes the `SET` the first statement. A failing `SET` destroys the connection. A pool cluster (`replication`) exposes no such hook and is returned unchanged.
- oracle: `extra.sessionCallback` (typeorm merges `extra` into `oracledb.createPool`) runs `ALTER SESSION SET TIME_ZONE`. node-oracledb decodes a zone-less `TIMESTAMP` into a LOCAL Date with no switch (`readOracleDate(useLocalTime)` is decided by the column type) and exposes `fetchTypeHandler` only process-wide or per `execute()`, so `driver` becomes a proxy whose pools hand out proxied connections: `execute` / `queryStream` add a handler re-reading the local fields as UTC (a caller's or the process-wide handler decides first), and `execute` / `executeMany` / `queryStream` bind `Date` parameters as `DB_TYPE_TIMESTAMP_TZ`, which sends the instant rather than local fields. Fetching as a string was tried and is useless in thin mode (it stringifies the local Date, and mangles years 0-99). The one inexactness: in a process zone with daylight saving, a UTC value whose fields fall into the local spring-forward gap reads an hour late, since the driver's local Date already normalized it. The proxy binds methods to their target so connection internals never re-enter it, and passes through any member carrying a `prototype` unless it is a replaced method (exported classes keep `instanceof`; note `oracledb.createPool` is a plain function WITH a prototype, which is why replaced methods are checked first).

The folder is split like the create/drop layer: `module.ts` validates the zone and dispatches on `options.type` to one `apply<Driver>Timezone()` per driver file (`postgres.ts`, `mysql.ts`, `oracle.ts`), each owning its wrappers and its all-or-nothing guard; `type.ts` holds the public `DataSourceTimezone` plus the internal driver shapes, and `utils.ts` the shared guards. The barrel re-exports by NAME, not `export *`: `isDate`, the driver shapes and the `apply*` functions stay internal, so splitting the file changed no public export. A new driver is one file plus one `case`.

All or nothing per driver: anything already set on either side (mysql `timezone`; postgres `TimeZone` startup option, `extra.types`, `extra.Client`; oracle `extra.sessionCallback`) returns the options unchanged, which is also what makes the call idempotent. The driver modules come from `options.driver` or typeorm's own `PlatformTools.load`, so every wrapper belongs to the copy typeorm uses. Only UTC is supported: reading any other zone needs the offset in force at each instant. Untouched on purpose: typeorm maps cockroachdb date columns to `timestamptz` (checked against a live server: correct unpinned), better-sqlite3 stamps and reads UTC (held by `test/unit/data-source/options/timezone/sqlite.spec.ts`), mongodb stores UTC; mssql stamps `getdate()` (server OS time, no session zone to set), so only a column default such as `SYSUTCDATETIME()` fixes it.

`DB_TIMEZONE` / `TYPEORM_TIMEZONE` request it from the environment. It is applied once, AFTER `mergeDataSourceOptionsWithEnv` has merged: the smob merge is deep, so applying it to the env half first would merge into a wrapped driver module. `readDataSourceOptionsFromEnv` applies it to its own result, and the merge reads the raw env options for that reason.
