...
| `test/integration/database/error.spec.ts`   | the three driver error classifiers against the errors a real server raises: a duplicate unique column and primary key, a missing referenced row, the delete of a referenced row, and the victim of a deadlock between two sessions (each rolls the victim back, and swallows the rollback error, since mssql has already aborted it). The deadlock case skips cockroachdb, where the victim did not surface within the timeout against v24.3.8, `maxTransactionRetries: 0` included; mongodb runs only the duplicate key. `test/unit/database/error.spec.ts` holds every code shape and a real better-sqlite3 run |
