...
| `test/integration/data-source/timezone.spec.ts` | `withDataSourceTimezone` on postgres / mysql / mariadb / oracle, with the process at UTC-10 and the postgres / mysql database default far from UTC (the server needs the privilege: `ALTER DATABASE` / `SET GLOBAL`; a database-level postgres default it finds is restored, mariadb stops below `+14:00`, hence `+13:00`). It asserts the STORED wall clock by SQL, not only what reads back, because oracle unpinned reads correctly while storing the process's wall clock. The control proves the stored clock is off UTC unpinned; pinned, a database-stamped and an app-written value both store UTC and read back exact |
