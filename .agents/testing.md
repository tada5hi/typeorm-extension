...
| `test/integration/database/error.spec.ts`   | the three driver error classifiers against the errors a real server raises: a duplicate unique column and primary key, a missing referenced row, the delete of a referenced row, and the victim of a deadlock between two sessions (each rolls the victim back, and swallows the rollback error, since mssql has already aborted it). On cockroachdb the suite sets `maxTransactionRetries: 0`: typeorm's query runner otherwise replays the victim on `40001`, the victim wins after its restart and the other session waits on it, so no error surfaces and the test hangs. mongodb runs only the duplicate key. `test/unit/database/error.spec.ts` holds every code shape and a real better-sqlite3 run |
| `test/integration/data-source/timezone.spec.ts` | `pinTimezone` on postgres / mysql / mariadb / oracle, with the process at UTC-10 and the postgres / mysql database default far from UTC (the server needs the privilege: `ALTER DATABASE` / `SET GLOBAL`; a database-level postgres default it finds is restored, mariadb stops below `+14:00`, hence `+13:00`; every cleanup step runs whatever the one before did). It asserts the STORED wall clock by SQL, plus: a `date` column round trip, the values `save()` hands back (oracle `RETURNING` out-binds), postgres `timestamp[]`, stream parameters (`pg-query-stream` is a devDependency for this), a url carrying `options` and `PGOPTIONS` (via `SHOW`), oracle `executeMany`, and an oracle plan free of column-side conversion. Each fix was mutated out once and the suite failed on its driver. What needs no server (conflicts, url surgery, the parsers, `bindDefs`, untyped INOUT binds, the standalone oracle pin, altered-pin and env re-checks) is held by `test/unit/data-source/options/timezone/`, mutation-checked the same way |
